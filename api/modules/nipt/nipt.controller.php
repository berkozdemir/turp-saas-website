<?php
/**
 * Module: NIPT Controller
 * Endpoint: ?action=nipt_init, ?action=calculate_booking_price
 * Tenant: NIPT (Validation Required)
 * Description:
 * - Orchestrates NIPT booking flow (Pre-chat -> Form -> Payment).
 * - Handles patient authentication via TR ID/Passport.
 * Related:
 * - Service: email_service.php
 */
// NIPT API Handler
...
");
        ...
        try {
            ...
            $conn->exec("ALTER TABLE nipt_bookings ADD CONSTRAINT fk_booking_referral FOREIGN KEY (referral_code_id) REFERENCES
referral_codes(id) ON DELETE SET NULL");
        } catch (Exception $e) {
        ...
        try {
            $conn->exec("ALTER TABLE nipt_bookings MODIFY COLUMN sample_collection_method ENUM('courier', 'clinic', 'home_care')
DEFAULT 'courier'");
        } catch (Exception $e) {
...
(tenant_id, patient_id, booking_date, booking_time, sample_collection_method, sample_collection_address,
clinic_location, status, kvkk_consent, test_terms_consent, referral_code_id)
VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
");
...
if ($action == 'validate_referral' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("SELECT id, code, doctor_name, discount_percent FROM referral_codes WHERE code = ? AND is_active
= 1");
        $stmt->execute([$code]);
