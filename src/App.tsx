...
const COMPANY_INFO = {
    ...
    email: "info@turp.app",
    copyright: "© 2025 Turp Sağlık Teknolojileri A.Ş."
};
...
// --- MODÜL İÇERİĞİ OLUŞTURUCU (ÇEVİRİYE DUYARLI) ---
const getModuleContent = (t) => ({
  'survey': {
    title: t("mod_survey_title"), short: t("mod_survey_short"), heroDesc: t("mod_survey_desc"),
    icon: ClipboardList, color: "from-blue-600 to-indigo-600",
    image: "https://zhljpirfyiuraeocgbep.supabase.co/storage/v1/object/public/blog-images/survey.jpg",
    details: [t("mod_survey_d1"), t("mod_survey_d2"), t("mod_survey_d3")],
    features: [{t:"Logic", d:t("mod_survey_d3")}, {t:"Timestamp", d:t("mod_survey_d1")}]
  },
  'medication': {
    title: t("mod_med_title"), short: t("mod_med_short"), heroDesc: t("mod_med_desc"),
    icon: Bell, color: "from-green-500 to-emerald-700",
    image: "https://zhljpirfyiuraeocgbep.supabase.co/storage/v1/object/public/blog-images/medication.jpg",
    details: [t("mod_med_d1"), t("mod_med_d2"), t("mod_med_d3")],
    features: [{t:"Visual", d:t("mod_med_d1")}, {t:"Stock", d:t("mod_med_d3")}]
  },
  'vital': {
    title: t("mod_vital_title"), short: t("mod_vital_short"), heroDesc: t("mod_vital_desc"),
    icon: HeartPulse, color: "from-rose-500 to-red-700",
    image: "https://zhljpirfyiuraeocgbep.supabase.co/storage/v1/object/public/blog-images/vital.jpg",
    details: [t("mod_vital_d1"), t("mod_vital_d2"), t("mod_vital_d3")],
    features: [{t:"BYOD", d:t("mod_vital_d2")}, {t:"Alert", d:t("mod_vital_d3")}]
  },
  'appointment': {
    title: t("mod_appt_title"), short: t("mod_appt_short"), heroDesc: t("mod_appt_desc"),
    icon: Calendar, color: "from-purple-600 to-violet-800",
    image: "https://zhljpirfyiuraeocgbep.supabase.co/storage/v1/object/public/blog-images/appointment.jpg",
    details: [t("mod_appt_d1"), t("mod_appt_d2"), t("mod_appt_d3")],
    features: [{t:"Reminder", d:t("mod_appt_d1")}, {t:"Nav", d:t("mod_appt_d3")}]
  },
  'adverse': {
    title: t("mod_adv_title"), short: t("mod_adv_short"), heroDesc: t("mod_adv_desc"),
    icon: AlertTriangle, color: "from-orange-500 to-amber-600",
    image: "https://zhljpirfyiuraeocgbep.supabase.co/storage/v1/object/public/blog-images/adverse.jpg",
    details: [t("mod_adv_d1"), t("mod_adv_d2"), t("mod_adv_d3")],
    features: [{t:"One-Click", d:t("mod_adv_d1")}, {t:"SAE Alert", d:t("mod_adv_d3")}]
  },
  'education': {
    title: t("mod_edu_title"), short: t("mod_edu_short"), heroDesc: t("mod_edu_desc"),
    icon: BookOpen, color: "from-sky-500 to-cyan-600",
    image: "https://zhljpirfyiuraeocgbep.supabase.co/storage/v1/object/public/blog-images/education.jpg",
    details: [t("mod_edu_d1"), t("mod_edu_d2"), t("mod_edu_d3")],
    features: [{t:"eConsent", d:t("mod_edu_d1")}, {t:"Video", d:t("mod_edu_d2")}]
  },
  'webinar': {
    title: t("mod_web_title"), short: t("mod_web_short"), heroDesc: t("mod_web_desc"),
    icon: Video, color: "from-fuchsia-600 to-pink-600",
    image: "https://zhljpirfyiuraeocgbep.supabase.co/storage/v1/object/public/blog-images/webinar.jpg",
    details: [t("mod_web_d1"), t("mod_web_d2"), t("mod_web_d3")],
    features: [{t:"Encrypted", d:t("mod_web_d1")}, {t:"Audit", d:t("mod_web_d3")}]
  }
});
...
const FAQItem = ({ question, answer }) => {
  ...
  return (
    <div className="border-b border-slate-200 last:border-0">
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center justify-between w-full py-6 text-left group">
    ...
    <footer className="bg-slate-900 text-white pt-20 pb-10 border-t border-slate-800">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div><h4 className="font-bold text-lg mb-6">Platform</h4><ul className="space-y-4 text-slate-400 text-sm"><li><button onClick={() => setView('home')} className="hover:text-white">{t("nav_home")}</button></li><li><button onClick={() => setView('blog')} className="hover:text-white">{t("nav_blog")}</button></li></ul></div>
          <div><h4 className="font-bold text-lg mb-6">Kurumsal</h4><ul className="space-y-4 text-slate-400 text-sm"><li><button onClick={() => setView('admin')} className="hover:text-white">{t("nav_admin")}</button></li></ul></div>
...
const ModuleDetail = ({ moduleId, setView }) => {
  const { t } = useTranslation();
  const data = getModuleContent(t)[moduleId]; // Çeviriyi al
  useEffect(() => { window.scrollTo(0, 0); }, [moduleId]);
  if (!data) return <div className="p-20 text-center">...</div>;
    ...
    <div className="min-h-screen bg-white animate-in fade-in slide-in-from-right-8 duration-500">
      <div className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
           <button onClick={() => setView('home')} className="flex items-center gap-2 text-slate-600 hover:text-slate-900 font-bold transition-colors"><ArrowLeft size={20}/> {t("module_back")}</button>
           <button onClick={() => document.getElementById('contact-module').scrollIntoView({behavior: 'smooth'})} className="px-5 py-2 bg-slate-900 text-white rounded-full text-sm font-bold hover:bg-slate-700 transition-colors shadow-lg">{t("module_btn_detail")}</button>
        </div>
      </div>
      <section className="relative py-32 px-6 overflow-hidden flex items-center justify-center min-h-[60vh]">
        <div className="absolute inset-0 z-0"><img src={data.image} alt={data.title} className="w-full h-full object-cover object-center" /><div className={`absolute inset-0 bg-gradient-to-r ${data.color} opacity-90 mix-blend-multiply`}></div></div>
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-start">
         <div><h2 className="font-heading text-3xl md:text-4xl font-bold text-slate-900 mb-8">{t("module_why")}</h2><p className="text-lg text-slate-600 leading-relaxed mb-10 font-medium border-l-4 border-slate-900 pl-6">{data.heroDesc}</p><div className="space-y-8">{data.details.map((detail, i) => (<div key={i} className="flex gap-5"><div className="mt-1.5 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-900 font-bold shrink-0">{i+1}</div><p className="text-slate-600 leading-relaxed text-lg">{detail}</p></div>))}</div></div>
         <div className="bg-slate-50 p-10 rounded-[2rem] border border-slate-200 shadow-xl relative overflow-hidden"><h3 className="font-heading text-2xl font-bold text-slate-900 mb-8 relative z-10">{t("module_tech")}</h3><div className="grid gap-6 relative z-10">{data.features.map((feat, i) => (<div key={i} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-start gap-4 hover:shadow-md transition-shadow"><div className={`p-3 rounded-xl bg-gradient-to-br ${data.color} text-white shrink-0 shadow-md`}><CheckCircle size={20}/></div><div><h4 className="font-bold text-slate-900 text-lg">{feat.t}</h4><p className="text-sm text-slate-500 mt-1">{feat.d}</p></div></div>))}</div></div>
      </section>
      <section id="contact-module" className="py-24 bg-slate-900 text-white text-center px-6"><div className="max-w-3xl mx-auto"><h2 className="font-heading text-4xl font-bold mb-6">{t("module_cta_title")}</h2><p className="text-slate-400 mb-10 text-lg">{t("module_cta_desc")}</p><button onClick={() => setView('home')} className="px-12 py-5 bg-white text-slate-900 font-bold rounded-2xl hover:scale-105 transition-transform shadow-xl text-lg">{t("module_cta_btn")}</button></div></section>
    </div>
  );
};
...
const Home = ({ setView }) => {
  const { t } = useTranslation();
  const modules = getModuleContent(t); // Modül verisini çek
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 overflow-hidden">
      ...
      <section className="py-24 px-6 bg-slate-50">
        <div className="max-w-6xl mx-auto">
            <div className="text-center mb-16"><h2 className="font-heading text-3xl md:text-4xl font-bold text-slate-900 mb-4">{t("prob_title")}</h2><p className="text-slate-500">{t("prob_desc")}</p></div>
      ...
      <section id="features" className="py-24 px-6 max-w-7xl mx-auto">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[300px]">
            {Object.entries(modules).map(([key, val], i) => {
                const isBig = i === 0 || i === 6; 
                return (
                    <div key={key} onClick={() => setView({ type: 'module', id: key })} className={`cursor-pointer ${isBig ? 'md:col-span-2 bg-gradient-to-br from-slate-800 to-slate-900 text-white' : 'bg-white border border-slate-200 shadow-lg hover:border-rose-400'} rounded-3xl p-8 relative overflow-hidden group hover:shadow-2xl hover:-translate-y-1 transition-all duration-300`}>
                        <div className="relative z-10"><div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 ${isBig ? 'bg-white/20 backdrop-blur' : 'bg-slate-100 text-slate-600'}`}>{React.createElement(val.icon, { size: 24 })}</div><h3 className={`text-2xl font-heading font-bold mb-3 ${isBig ? 'text-white' : 'text-slate-900'}`}>{val.title}</h3><p className={`${isBig ? 'text-slate-300' : 'text-slate-500'}`}>{val.short}</p></div>
                        {isBig && <div className="absolute right-0 bottom-0 w-64 h-64 bg-white/5 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-700"></div>}
      ...
      <section className="py-20 bg-rose-600 text-white">
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-12 text-center">
              {['1','2','3','4'].map(num => (<div key={num}><div className="text-4xl md:text-5xl font-heading font-extrabold mb-2">{num === '1' ? '%45' : num === '2' ? '%99.8' : num === '3' ? '%30' : '7/24'}</div><div className="text-rose-200 text-sm font-medium uppercase tracking-wide">{t(`stat_${num}`)}</div></div>))}
          </div>
      </section>
...
// Bu bileşenler önceki kodunuzda sabittir ve değişmedi. Kopyalarken önceki yanıtımdaki gibi yerleştirebilirsiniz.
// Önemli olan Home bileşenindeki ve getModuleContent fonksiyonundaki "t()" kullanımlarıdır.
// (Aşağıdakiler kısaltılmıştır, lütfen App.tsx'te önceki tam hallerini koruyun)
...
export default function App() {
  ...
  const modules = getModuleContent(t); // Modül verisi çekilirken t fonksiyonu kullanılır
