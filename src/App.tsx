...
const Login = () => {
  ...
  const [mode, setMode] = useState('login'); // 'login' veya 'reset'
  ...
  const handleLogin = async (e) => {
    ...
    // Başarılı olursa App bileşenindeki onAuthStateChange tetiklenir ve sayfa değişir.
    setLoading(false);
  };
  ...
  const handleResetPassword = async (e) => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: window.location.origin + '/update-password', // Şifre yenileme sayfası (basitlik için ana sayfaya döner)
    });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-6">
      <div className="bg-white p-10 rounded-3xl shadow-2xl shadow-slate-200 border border-slate-100 w-full max-w-md animate-in fade-in zoom-in duration-500">
        <div className="text-center mb-8">
          <div className="w-12 h-12 bg-rose-600 rounded-xl flex items-center justify-center text-white mx-auto mb-4 shadow-lg shadow-rose-200">
            <Lock size={24} />
          </div>
          ...
          <p className="text-slate-500 mt-2 text-sm">
            {mode === 'login' ? 'Devam etmek için giriş yapın.' : 'E-posta adresinizi girin.'}
          </p>
        </div>
        ...
        <form onSubmit={mode === 'login' ? handleLogin : handleResetPassword} className="space-y-5">
          <div>
            ...
            <div className="relative">
              ...
              <input 
                type="email" 
                className="w-full pl-12 pr-4 py-3 border-2 border-slate-200 rounded-xl focus:border-rose-500 outline-none transition-all font-medium" 
                placeholder="admin@turp.com"
                value={email}
          ...
          <button 
            disabled={loading}
            className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg hover:bg-rose-600 hover:shadow-lg hover:shadow-rose-600/30 transition-all transform active:scale-[0.98]"
          >
            {loading ? 'İşleniyor...' : (mode === 'login' ? 'Giriş Yap' : 'Bağlantı Gönder')}
          </button>
        </form>

        <div className="mt-6 text-center">
          <button 
            onClick={() => {setMode(mode === 'login' ? 'reset' : 'login'); setMessage({type:'',text:''});}}
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center mb-8">
        ...
        {/* Bu butonu sadece adminler için veya public bırakabiliriz, şimdilik public kalsın ama Login olmadan admin sayfasına gidilemez */}
...
const Admin = ({ editingPost, setEditingPost, setView }) => {
  ...
  const handleLogout = async () => {
    ...
    // Oturum durumu App bileşeninde onAuthStateChange ile algılanacak ve sayfayı yenileyecek
  };
    ...
    <div className="max-w-3xl mx-auto px-6 py-20">
      ...
      <div className="bg-white p-10 rounded-3xl shadow-xl border border-slate-200">
        <div className="flex justify-between items-center mb-10">
            <div>
               <h2 className="font-heading text-2xl font-bold text-slate-900">{editingPost ? 'Yazıyı Düzenle' : 'Yeni Yazı Ekle'}</h2>
               <p className="text-slate-500">Markdown formatında içerik girebilirsiniz.</p>
            </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Oturum değişikliğini dinle (Giriş/Çıkış anında tetiklenir)
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      if (!session && view === 'admin') {
          setView('home'); // Çıkış yapınca ana sayfaya at
      }
    });
  ...
  const startEdit = (post) => {
    if (!session) {
        alert("Düzenleme yapmak için giriş yapmalısınız!");
        setView('admin'); // Giriş ekranına yönlendir
        return;
    }
    setEditingPost(post);
    setView('admin');
  ...
  const renderView = () => {
    ...
    // ADMIN GÖRÜNÜMÜ: Eğer oturum varsa Admin paneli, yoksa Login ekranı
    if (view === 'admin') {
        return session ? <Admin editingPost={editingPost} setEditingPost={setEditingPost} setView={setView} /> : <Login />;
    }
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <div className="flex items-center gap-1 bg-white/80 backdrop-blur border border-slate-200 p-1.5 rounded-full shadow-sm">
              ...
              {languages.map((lang) => (<button key={lang.code} onClick={() => i18n.changeLanguage(lang.code)} className={`w-8 h-8 rounded-full text-xs font-bold flex items-center justify-center ${i18n.language === lang.code ? 'bg-rose-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-200'}`}>{lang.label}</button>))}
            </div>
          </div>
        </div>
      </nav>
