...
import { RheumaCaseStudy } from './pages/RheumaCaseStudy'; // Yeni Senaryo Sayfası

export default function App() {
  // --- STATE YÖNETİMİ ---
  const [view, setView] = useState('home'); // Hangi sayfa açık?
  const [editingPost, setEditingPost] = useState(null); // Admin'de hangi yazı düzenleniyor?
  const [session, setSession] = useState(null); // Kullanıcı giriş yapmış mı?
  ...
  useEffect(() => {
    ...
    const initLocalization = async () => {
      ...
      // Eğer dil farklıysa güncelle
      if (i18n.language !== settings.lang) {
        i18n.changeLanguage(settings.lang);
      }
      // Para birimini güncelle (ROI hesaplayıcıya gidecek)
  ...
  const startEdit = (post) => { 
      if (!session) { alert("Giriş yapmalısınız!"); setView('admin'); return; } 
      setEditingPost(post); 
      setView('admin'); 
  };
  ...
  const renderView = () => {
    switch (view) {
        ...
        // ROI Hesaplayıcıya otomatik algılanan para birimini gönderiyoruz
        case 'roi': return <ROICalculator initialCurrency={globalCurrency} />;
        // Yeni Senaryo Sayfası
        case 'case-rheuma': return <RheumaCaseStudy setView={setView} />;
        
        case 'admin': 
            return session ? <Admin editingPost={editingPost} setEditingPost={setEditingPost} setView={setView} handleLogout={handleLogout} /> : <Login />;
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            <div className="flex items-center bg-white/80 backdrop-blur border border-slate-200 p-1.5 rounded-full shadow-sm">
              ...
              <button onClick={()=>setView('home')} className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${view === 'home' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}>
                  {t("nav_home")}
              </button>
              ...
              {/* Diğer Linkler */}
              <button onClick={()=>setView('roi')} className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${view === 'roi' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}>
                  ROI
              </button>

              <button onClick={()=>setView('case-rheuma')} className={`hidden lg:flex items-center gap-1 px-4 py-2 rounded-full text-sm font-bold transition-all ${view === 'case-rheuma' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}>
                  <FileText size={14}/> Senaryolar
              </button>

              <button onClick={()=>setView('blog')} className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${view === 'blog' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}>
                  {t("nav_blog")}
              </button>

              <button onClick={()=>setView('about')} className={`hidden md:block px-4 py-2 rounded-full text-sm font-bold transition-all ${view === 'about' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}>
                  Hakkımızda
              </button>
              
              {/* Admin Butonu (Sadece Giriş Yapılmışsa Navbar'da göster, yoksa Footer'da) */}
              {session && (
                  <button onClick={()=>setView('admin')} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all ${view === 'admin' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}>
            ...
            <div className="flex items-center gap-1 bg-white/80 backdrop-blur border border-slate-200 p-1.5 rounded-full shadow-sm">
                ...
                {languages.map((lang) => (
                    <button 
                        ...
                        className={`w-8 h-8 rounded-full text-xs font-bold flex items-center justify-center transition-all ${i18n.language === lang.code ? 'bg-rose-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-200'}`}
                    >
                        {lang.label}
                    </button>
                ))}
