...
interface AdminContactConfigListProps {
    onEdit: (id: number | null) => void;
}

export const AdminContactConfigList = ({ onEdit }: AdminContactConfigListProps) => {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const token = localStorage.getItem("admin_token");
            const response = await fetch(`${API_URL}/index.php?action=delete_contact_config`, {
                method: "POST",
                headers: {
    ...
    const handleToggleActive = async (config: ContactConfig) => {
        try {
            const token = localStorage.getItem("admin_token");
    ...
    return (
        <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800">İletişim Ayarları</h2>
                    <p className="text-slate-500">İletişim sayfası yapılandırmaları</p>
                </div>
                <button
                    onClick={() => onEdit(null)}
                    ...
                    <table className="w-full">
                        <thead className="bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-sm font-semibold text-slate-600">Şehir</th>
                        ...
                        <tbody>
                            {configs.map((config) => (
                                <tr key={config.id} className="border-b border-slate-100 hover:bg-slate-50">
                                    ...
                                    <td className="px-4 py-3">
                                        <span className="text-sm text-slate-600">{config.city || '-'}</span>
                                    </td>
